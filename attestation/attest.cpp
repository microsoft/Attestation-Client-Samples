...
std::string get_tenant_id() { return std::string(""); } // Tenant ID for the Azure account.
std::string get_client_id() { return std::string(""); } // The Client ID to authenticate the request.
std::string get_client_secret() { return std::string(""); } // The client secret.
std::string get_maa_provider_uri() { return std::string("https://prakharnoauthtest.eus.attest.azure.net"); } // Microsoft Azure Attestation provider's Attest URI (as shown in portal). Format is similar to "https://<ProviderName>.<Region>.attest.azure.net".
...
vector<uint8_t> send_to_att_service(const uint8_t* data, size_t size)
{
    try
    {
        ...
        auto att_client = AttestationClient::Create(get_maa_provider_uri(), NULL);
