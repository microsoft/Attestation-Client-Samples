...
wil::unique_ncrypt_key create_key(PCWSTR provider_name, PCWSTR key_name, DWORD flags)
{
    ...
    THROW_IF_FAILED_MSG(NCryptOpenStorageProvider(&provider_handle, provider_name, 0), "Error opening storage provider.");

    wil::unique_ncrypt_key key{};
    THROW_IF_FAILED_MSG(NCryptCreatePersistedKey(provider_handle.get(), &key, BCRYPT_RSA_ALGORITHM, key_name, 0, flags), "Error creating key.");
...
wil::unique_ncrypt_key create_tpm_key(const PCWSTR& name, bool machine_key)
{
    ...
    wil::unique_ncrypt_key tpm_key = create_key(MS_PLATFORM_KEY_STORAGE_PROVIDER, name, NCRYPT_OVERWRITE_KEY_FLAG | (machine_key ? NCRYPT_MACHINE_KEY_FLAG : 0));
...
wil::unique_ncrypt_key create_ephemeral_software_key()
{
    ...
    wil::unique_ncrypt_key ephemeral_software_key = create_key(MS_KEY_STORAGE_PROVIDER, NULL, 0);
